...
- `chrome.storage.sync` som komplement till `local` för synk mellan egna Chrome-profiler (obs ~100KB-gräns).
...
- Ändringslogg ("vad är nytt") vid uppdatering av tillägget.

## Kodkvalitet/underhåll
- Enhetstester (t.ex. jsdom) för URL-matchning/sanering (`getHostname`, `buildEquivalentUrl`, `sanitizeUrl`).
- ESLint + byggskript som paketerar `.zip` och bumpar `manifest.json`-version inför release.
